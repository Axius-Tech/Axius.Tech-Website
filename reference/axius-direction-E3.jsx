// ============================================================
// AXIUS · DIRECTION E3 — QUIET THREE (PHOSPHOR · GRAPHS)
//
// Quiet's tranquility, leaning further into Matrix:
//   • Phosphor green is the PRIMARY accent (CTAs, status, scroll)
//   • Deep green-tinted near-black canvas; ink is faintly phosphor
//   • Hero stat widgets get small sparklines (the "graph" moment)
//   • A small Dispatch ledger docked at the bottom-right streams
//     real operational lines — Matrix's event log, made quiet
//   • All of Quiet's hover behaviors carry through (latent colors,
//     founder photo dev/colour, cursor orb, ticker, count-ups)
// Restraint stays the rule. No code rain. No CRT chaos. Just
// enough Matrix to feel alive.
// ============================================================

window.AxiusDirectionE3 = function () {
  ...
  React.useEffect(() => {
    ...
    if (!document.getElementById('axius-quiet3-styles')) {
      const style = document.createElement('style');
      style.id = 'axius-quiet3-styles';
      style.textContent = `
        @keyframes axQ3Breathe {
        ...
        .ax-quiet3-scroll { scrollbar-width: thin; scrollbar-color: rgba(80,255,170,0.45) transparent; }
        .ax-quiet3-scroll::-webkit-scrollbar { width: 8px; }
        .ax-quiet3-scroll::-webkit-scrollbar-track { background: transparent; }
        .ax-quiet3-scroll::-webkit-scrollbar-thumb {
          background: rgba(80,255,170,0.40);
          border-radius: 0;
        }
        .ax-quiet3-scroll::-webkit-scrollbar-thumb:hover {
          background: rgba(80,255,170,0.68);
        }
  ...
  // ─── Palette (phosphor-leaning) ────────────────────────────
  // Quiet Three carries Matrix's primary register: deep green-tinted
  // near-black, slightly phosphor-tinted cream ink, phosphor green
  // as the lead action color. Warm accents stay as latent secondary
  // hues on hover.
  const C = {
    bg:        '#050805',
    surface:   '#0B1109',
    ...
    raised:    '#0F1A11',
    ink:       '#E5FBE9',
    dim:       'rgba(229,251,233,0.66)',
    mute:      'rgba(229,251,233,0.42)',
    faint:     'rgba(229,251,233,0.24)',
    ghost:     'rgba(229,251,233,0.12)',
    line:      'rgba(229,251,233,0.08)',
    lineHi:    'rgba(229,251,233,0.18)',
    // primary accent — phosphor green (the lead Matrix borrowing)
    phosphor:    '#50FFAA',
    phosphorHi:  '#9CFFCD',
    phosphorDeep:'#2DC07F',
    phosphorFaint:'rgba(80,255,170,0.14)',
    // latent secondary accents — kept for hover variety
    tangerine: '#FF7E55',
    lavender:  '#A39BFF',
    mint:      '#7AE7C9',
    amber:     '#FFDFA0',
  ...
  // Dispatch ledger — a tiny fixed dock at the bottom-right that
  // streams real-feel operational events. Matrix's event log, made
  // quiet: cream-on-dark mono, phosphor accents only on the timestamp
  // and pulsing status dot. Collapsible.
  const Dispatch = () => {
    ...
    const [events, setEvents] = React.useState(() => {
      ...
      return [
        { time: fmt(new Date(t - 12 * 60 * 1000)), text: 'Stripe → Notion sync · 12 records' },
        { time: fmt(new Date(t -  6 * 60 * 1000)), text: 'monthly_report.md filed' },
        { time: fmt(new Date(t -  1 * 60 * 1000)), text: 'voice agent · 1 lead qualified' },
      ];
    });

    React.useEffect(() => {
      const pool = [
        'stripe webhook routed',
        'missed_call.recovery triggered',
        'monthly_report.md filed for client 02',
        'config_map.json amended',
        'voice agent · 1 lead qualified',
        'doc.automation · proposal sent',
        'crm sync · 12 records updated',
        'dashboard · revenue +4.2%',
        'lead.scoring · model retrained',
        'inbox.unification · 3 channels merged',
        'ai.faq.bot · 2 questions answered',
        'churn.signal · early warning logged',
        'kb.agent · 14 new docs indexed',
        'a/b.test · variant B +18% CR',
        'video.repurpose · 4 reels shipped',
        'ltv model · 12.4mo avg',
      ];
      const id = setInterval(() => {
        const msg = pool[(Math.random() * pool.length) | 0];
        const t = new Date();
        const time = t.toTimeString().slice(0, 5);
        setEvents(e => [...e.slice(-18), { time, text: msg }]);
      }, 3600 + Math.random() * 1400);
      return () => clearInterval(id);
    }, []);
    ...
    return (
      <div style={{
        ...
        boxShadow: `0 0 0 1px rgba(80,255,170,0.08), 0 6px 24px rgba(0,0,0,0.4)`,
      }}>
        <div style={{
          padding: '8px 14px',
          ...
          <div style={{display: 'inline-flex', alignItems: 'center', gap: 8}}>
            <span style={{
              ...
              animation: 'axQ3Pulse 2.4s ease-out infinite',
            }}/>
            <span style={{
              fontFamily: MONO, fontSize: 10, fontWeight: 500,
          ...
          <button onClick={() => setCollapsed(c => !c)} style={{
            ...
            cursor: 'pointer', color: C.phosphor, fontFamily: MONO, fontSize: 12,
            padding: 0,
          }}>{collapsed ? '◢' : '◤'}</button>
        </div>
        {!collapsed && (
          <div ref={scrollRef} className="ax-quiet3-scroll" style={{
            padding: '6px 14px',
            height: 152, overflowY: 'auto',
            fontFamily: MONO, fontSize: 10.5, lineHeight: 1.85,
          }}>
            {events.map((e, i) => (
              <div key={i} style={{display: 'grid', gridTemplateColumns: '44px 1fr', gap: 8}}>
                <span style={{
                  color: C.phosphorDeep || C.phosphor, fontVariantNumeric: 'tabular-nums',
                }}>{e.time}</span>
                <span style={{color: C.dim}}>{e.text}</span>
              </div>
            ))}
          </div>
        )}
  ...
  const HoverHead = ({ accent = C.phosphor, style = {}, prefix, italic, suffix, italicStyle = {} }) => {
    const [h, setH] = React.useState(false);
    return (
      <h2
        onMouseEnter={() => setH(true)}
  ...
  const QuietLink = ({ children, accent = C.phosphor, style = {}, ...rest }) => {
    const [h, setH] = React.useState(false);
    return (
      <a {...rest}
        onMouseEnter={() => setH(true)}
  ...
  const QuietBtn = ({ children, primary, accent = C.phosphor, size = 'md', onClick, style = {} }) => {
    const [h, setH] = React.useState(false);
    const sizes = {
  ...
  const QuietCard = ({ accent = C.phosphor, children, style = {}, padding = 28, onClick, fillOnHover }) => {
    const [h, setH] = React.useState(false);
    return (
      <div
        onClick={onClick}
  ...
  const Nav = () => (
    <>
      ...
      <div aria-hidden className="ax-quiet3-scan" style={{
    ...
    return (
      <header ref={heroRef} data-screen-label="00 Hero" style={{
        ...
        <div aria-hidden style={{
          ...
          background: `radial-gradient(closest-side, rgba(80,255,170,0.18), rgba(80,255,170,0) 70%)`,
          animation: 'axQ3Glow 9s ease-in-out infinite',
        ...
        <h1
          ...
          <span style={{position: 'relative', display: 'inline-block', color: C.dim}}>
            ...
            <span aria-hidden style={{
              ...
              background: `linear-gradient(180deg, ${C.phosphor} 0%, ${C.phosphorHi} 100%)`,
              transformOrigin: 'left',
        ...
        <div style={{
          ...
          {[
            { v: <><CountUp to={129}/></>, k: 'workflows operated',   accent: C.phosphor,
              spark: [42, 56, 50, 64, 76, 82, 92, 104, 112, 119, 124, 129] },
            { v: '9',                       k: 'categories',           accent: C.lavender,
              spark: [3, 4, 5, 5, 6, 7, 8, 8, 9, 9, 9, 9] },
            { v: '< 24h',                   k: 'response · tier 3',    accent: C.mint,
              spark: [72, 68, 60, 55, 52, 48, 46, 44, 42, 40, 39, 38] },
            { v: '$1k',                     k: 'starts at, per month', accent: C.amber,
              spark: [10, 12, 14, 16, 18, 20, 22, 22, 22, 22, 22, 22] },
          ].map((s, i, arr) => (
            <HoverCell key={i} accent={s.accent}
              ...
              <div style={{
                ...
                <div style={{
                  fontFamily: DISPLAY, fontWeight: 600, fontSize: 48,
                  letterSpacing: '-0.045em', lineHeight: 1, color: C.ink,
                  fontVariantNumeric: 'tabular-nums',
                }}>{s.v}</div>
                <Sparkline data={s.spark} width={108} height={32} color={s.accent}/>
              </div>
            </HoverCell>
          ))}
        </div>
  ...
  // Sparkline — small SVG line chart with a final-point dot.
  // The line glows softly via drop-shadow when its accent is phosphor.
  const Sparkline = ({ data, width = 120, height = 32, color }) => {
    if (!data || !data.length) return null;
    const stroke = color || C.phosphor;
    ...
    return (
      <svg width={width} height={height} style={{display: 'block', overflow: 'visible'}}>
        <polyline points={pts} fill="none" stroke={stroke} strokeWidth={1.2}
          ...
          style={{filter: `drop-shadow(0 0 3px ${stroke})`, opacity: 0.85}}/>
        ...
        {(() => {
          ...
          return (
            <div style={{
              ...
              <div className="ax-quiet3-scroll" style={{
    ...
    return (
      <figure
        ...
        <figcaption style={{
          ...
          <span style={{
            fontFamily: MONO, fontSize: 10, color: h ? C.phosphor : C.mute,
            letterSpacing: '0.18em', textTransform: 'uppercase',
  ...
  const CTA = () => (
    <section data-screen-label="10 CTA" style={{
      ...
      <div aria-hidden style={{
        ...
        background: `radial-gradient(closest-side, rgba(80,255,170,0.13), rgba(80,255,170,0) 70%)`,
        animation: 'axQ3Glow 11s ease-in-out infinite',
        pointerEvents: 'none',
      }}/>
